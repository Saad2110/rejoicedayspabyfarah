...
if ( is_a( $product_object, 'WC_Product_PW_Gift_Card' ) ) {
    ...
    $product_ids = $product_object->get_pwgc_product_ids( 'edit' );
    $exclude_product_ids = $product_object->get_pwgc_exclude_product_ids( 'edit' );
    $product_categories = $product_object->get_pwgc_product_categories( 'edit' );
    $exclude_product_categories = $product_object->get_pwgc_exclude_product_categories( 'edit' );
    $exclude_tax = $product_object->get_pwgc_exclude_tax( 'edit' );
    $exclude_shipping = $product_object->get_pwgc_exclude_shipping( 'edit' );
    $redemption_restrictions = ( !empty( $product_ids ) || !empty( $exclude_product_ids ) || !empty( $product_categories ) || !empty( $exclude_product_categories ) || $exclude_shipping || $exclude_tax );
...
<div id='<?php echo PWGC_PRODUCT_TYPE_SLUG; ?>_cards' class='panel woocommerce_options_panel' style="display: none;">
    ...
    <div class='options_group show_if_<?php echo PWGC_PRODUCT_TYPE_SLUG; ?>'>
        <?php
            ...
            woocommerce_wp_text_input( array(
                ...
                'desc_tip'          => 'true',
                'description'       => __( 'The available fixed amounts that can be purchased.', 'pw-woocommerce-gift-cards' ),
            ) );
        ...
        <?php
            ...
            if ( empty( $default_country ) || strpos( $default_country, 'US:' ) === 0 ) {
                ...
                <div id="pwbf-expire-days-us-warning" class="pwbf-form-text">
                    <?php
                        // translators: %s is a link to the law for US-based stores.
    ...
    <div class="options_group show_if_<?php echo PWGC_PRODUCT_TYPE_SLUG; ?>">
        <?php
            woocommerce_wp_checkbox( array(
                'id'                => '_pwgc_redemption_restrictions',
                'value'             => $redemption_restrictions ? 'yes' : 'no',
                'cbvalue'           => 'yes',
                'label'             => __( 'Redemption restrictions?', 'pw-woocommerce-gift-cards' ),
                'description'       => __( 'Specify which products can be purchased using this gift card.', 'pw-woocommerce-gift-cards' ) . ' ' . __( 'Can also be set at the Variation level.', 'pw-woocommerce-gift-cards' ),
            ) );
        ?>
        <div id="pwgc_redemption_restrictions_container" class="<?php echo $redemption_restrictions ? '' : 'pwgc-hidden'; ?>">
            <p class="form-field">
                <label for="_pwgc_product_ids"><?php esc_html_e( 'Products', 'woocommerce' ); ?></label>
                <select id="_pwgc_product_ids" name="_pwgc_product_ids[]" style="width: 50%;" class="wc-product-search" multiple="multiple" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'woocommerce' ); ?>" data-action="woocommerce_json_search_products_and_variations" data-exclude="<?php echo intval( $product_object->get_id() ); ?>">
                    <?php
            ...
            <p class="form-field">
                <label for="_pwgc_exclude_product_ids"><?php esc_html_e( 'Exclude products', 'woocommerce' ); ?></label>
                <select id="_pwgc_exclude_product_ids" name="_pwgc_exclude_product_ids[]" style="width: 50%;" class="wc-product-search" multiple="multiple" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'woocommerce' ); ?>" data-action="woocommerce_json_search_products_and_variations" data-exclude="<?php echo intval( $product_object->get_id() ); ?>">
                    <?php
            ...
            <p class="form-field">
                <label for="_pwgc_product_categories"><?php _e( 'Product categories', 'woocommerce' ); ?></label>
                <select id="_pwgc_product_categories" name="_pwgc_product_categories[]" style="width: 50%;" class="wc-enhanced-select" multiple="multiple" data-placeholder="<?php esc_attr_e( 'Any category', 'woocommerce' ); ?>">
                    <?php
            ...
            <p class="form-field">
                <label for="_pwgc_exclude_product_categories"><?php _e( 'Exclude categories', 'woocommerce' ); ?></label>
                <select id="_pwgc_exclude_product_categories" name="_pwgc_exclude_product_categories[]" style="width: 50%;" class="wc-enhanced-select" multiple="multiple" data-placeholder="<?php esc_attr_e( 'No categories', 'woocommerce' ); ?>">
                    <?php
                        if ( is_array( $categories ) ) {
                            foreach ( $categories as $cat ) {
    ...
    <div class="options_group show_if_<?php echo PWGC_PRODUCT_TYPE_SLUG; ?>">
        ...
        ?>
        <p class="form-field _pwgc_email_design_field ">
            ...
            <?php
                ...
                $designer_url = pwgc_admin_url( 'designer' );
            ?>
        </p>
        <p class="form-field">
                            ...
                            <div class="pwgc-bonus-amount-container" data-key="<?php echo $spend; ?>">
                                ...
                                <span class="pwgc-bonus-amount">
                                    <?php
                                        ...
                                        printf( __( 'Buy a %1$s gift card, get a %2$s bonus card', 'pw-woocommerce-gift-cards' ), $pw_gift_cards->pretty_price( $spend ), $pw_gift_cards->pretty_price( $get ) );
                                    ?>
                                </span>
                            </div>
                            <?php

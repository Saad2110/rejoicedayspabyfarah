...
pimwick_define( 'PWGC_SHOW_SKU_IN_VARIATION_NAME', false );
pimwick_define( 'PWGC_REST_API_NOTES_ARE_MESSAGES', true );
pimwick_define( 'PWGC_INCLUDE_JS_GLOBALLY', false );
...
final class PW_Gift_Cards {
    ...
    function __construct() {
        ...
        if ( is_admin() ) {
            require_once( 'includes/class-pwgc-license.php' );
            $this->license = new PWGC_License( __FILE__ );
        }

        require_once( 'includes/pimwick-system-status-report.php' );
        add_action( 'pimwick_system_status_report', function() { pimwick_system_status_report_license_info( __FILE__ ); } );
    ...
    function woocommerce_init() {
        ...
        pimwick_define( 'PWGC_EMAIL_DESIGN_META_DISPLAY_NAME', __( 'Design', 'pw-woocommerce-gift-cards' ) );
        ...
        if ( isset( $_REQUEST['pwgc_number'] ) ) {
            ...
            if ( isset( $_REQUEST['qr_code'] ) ) {
                add_action( 'send_headers', array( $this, 'output_qr_code' ), PWGC_SEND_HEADERS_PRIORITY );
            } else {
                // Price Based on Country for WooCommerce by Oscar Gare
                if ( class_exists( 'WCPBC_Pricing_Zones' ) ) {
                    if ( $_REQUEST['pwgc_number'] != pwgc_get_example_gift_card_number() ) {
        ...
        if ( 'yes' === get_option( 'pwgc_allow_scheduled_delivery', 'yes' ) ) {
            ...
            if ( !$next_scheduled ) {
                $scheduled_time = apply_filters( 'pwgc_scheduled_start_time', false );
                if ( $scheduled_time !== false ) {
                    // The hook was originally called pwgc_scheduled_start_time. Maintain backwards compatibility
                    // in case someone has overridden this value on their site.
                    wp_schedule_event( $scheduled_time, 'daily', 'pw_gift_cards_delivery' );
                } else {
                    // Schedule a single event instead of a "daily" event (every 24 hours) to account for daylight saving time.
                    // See discussion here: https://stackoverflow.com/questions/53158188/wp-schedule-event-and-daylight-savings
                    $scheduled_time = apply_filters( 'pwgc_scheduled_email_start_time', pwgc_strtotime( 'tomorrow midnight' ) );
                    wp_schedule_single_event( $scheduled_time, 'pw_gift_cards_delivery' );
                }
            }
    ...
    function wp_enqueue_scripts() {
        ...
        if ( 'yes' === get_option( 'pwgc_allow_scheduled_delivery', 'yes' ) && is_a( $product, 'WC_Product_PW_Gift_Card' ) ) {
        ...
        if ( PWGC_INCLUDE_JS_GLOBALLY ) {
            wp_enqueue_script( 'pw-gift-cards' );
        }

        wp_register_script( 'pikaday', $this->relative_url( '/assets/js/pikaday-1.8.2.min.js' ), array( 'moment-with-locales' ), PWGC_VERSION );
        wp_register_style( 'pikaday', $this->relative_url( '/assets/css/pikaday-1.8.2.min.css' ), array(), PWGC_VERSION );
    ...
    function get_gift_card_product() {
        _deprecated_function( __FUNCTION__, '1.393', 'pwgc_get_gift_card_product' );
        return pwgc_get_gift_card_product();
    }

    function get_balance_page() {
        _deprecated_function( __FUNCTION__, '1.393', 'pwgc_get_balance_page' );
        return pwgc_get_balance_page();
    }
    ...
    function output_html_as_pdf( $html ) {
        ...
        // Allow plugins to override the chosen temporary directory.
        $tempDir = apply_filters( 'pwgc_mpdf_temp_dir', get_temp_dir() . 'pw-gift-cards-pdf' );

        if ( !is_dir( $tempDir ) ) {
            @mkdir( $tempDir );
        }

        // If we can't use the default temporary directory, we fall back to using
        // a folder inside the plugin directory.
        if ( !is_writable( $tempDir ) ) {
            $tempDir = PWGC_PLUGIN_ROOT . 'temp';
            if ( !is_dir( $tempDir ) ) {
                @mkdir( $tempDir );
            }
        }
    ...
    function output_qr_code() {
        ...
        $design = $pw_gift_cards_email_designer->get_design_by_id( $design_id );

        $size = is_array( $design ) ? absint( $design['qr_code_size'] ) : 200;
        ...
        $redeem_url = pwgc_redeem_url( $item_data );

        $redeem_url = apply_filters( 'pwgc_qr_code_url', $redeem_url, $gift_card_number, $design_id );

        $ch = curl_init();
        curl_setopt( $ch, CURLOPT_URL, 'http://chart.apis.google.com/chart' );
        curl_setopt( $ch, CURLOPT_POST, true );
        curl_setopt( $ch, CURLOPT_POSTFIELDS, "chs={$size}x{$size}&cht=qr&chl=" . urlencode( $redeem_url ) );
        ...
        if ( $png ) {
            ...
            header( 'Content-Disposition: attachment; filename=pw-gift-cards-qr-code.png' );
    ...
    function output_gift_card() {
        ...
        // Get the gift card object if possible.
        $gift_card = false;
        if ( $gift_card_number != pwgc_get_example_gift_card_number() ) {
            ...
            if ( !$gift_card->get_id() ) {
                ...
                wp_die( __( 'Error', 'pw-woocommerce-gift-cards' ) . ': ' . $gift_card->get_error_message() );
            }
        }
        ...
        if ( empty( $variation_id ) && !empty( $gift_card ) ) {
            $variation_id = $gift_card->get_variation_id();
        }
        $item_data->product = wc_get_product( $variation_id );
        if ( !empty( $item_data->product ) && is_a( $item_data->product, 'WC_Product' ) ) {
            $product_id = !empty( $item_data->product->get_parent_id() ) ? $item_data->product->get_parent_id() : $item_data->product->get_id();
            $item_data->parent_product = wc_get_product( $product_id );
        }
        ...
        if ( !empty( $gift_card ) ) {
            ...
            if ( !empty( $gift_card->get_expiration_date() ) ) {
            ...
            } else if ( !empty( $item_data->product ) && method_exists( $item_data->product, 'get_price' ) && !empty( $item_data->product->get_price() ) ) {
                $item_data->amount = $item_data->product->get_regular_price();
            } else {
                $item_data->amount = '123.45';
            }

            if ( empty( $expiration_date ) ) {
                $expires_in_days = is_a( $item_data->product, 'WC_Product_PW_Gift_Card' ) ? absint( $item_data->product->get_pwgc_expire_days() ) : 0;

...
final class PW_Gift_Cards_Shortcodes {
            ...
            if ( $gift_card->get_active() ) {
                ...
                if ( !empty( $expiration_date ) ) {
                    ...
                    if ( $gift_card->has_expired() ) {
                        $expiration_date .= '<div style="color: red; font-weight: 600;">' . __( 'Expired', 'pw-woocommerce-gift-cards' ) . '</div>';
                    }
                }
    ...
    function ajax_debit() {
        ...
        if ( $gift_card->get_id() ) {
            ...
            // Do not add a transaction if nothing is passed in.
            if ( !empty( $amount ) || !empty( $note ) ) {

                // Add the user's IP address and a note explaining where this transaction originated.
                // translators: %s is the user's IP address.
                $note .= ' ' . sprintf( __( "Manual debit from the Check Balance page. User's IP Address: %s", 'pw-woocommerce-gift-cards' ), pwgc_get_users_ip() );
                $note = trim( $note );
                $note = apply_filters( 'pwgc_manual_debit_note', $note, $gift_card, $amount );
    ...
    function woocommerce_account_menu_items( $items, $endpoints ) {
        ...
        // Prevent a fatal error in case someone has changed the type of the $items array using a hook.
        if ( !is_array( $items ) ) {
            return;
        }
    ...
    function woocommerce_get_endpoint_url( $url, $endpoint, $value, $permalink ) {
        ...
        if ( $endpoint == 'pw-gift-card-balance' ) {
            return get_permalink( pwgc_get_balance_page() );
        }

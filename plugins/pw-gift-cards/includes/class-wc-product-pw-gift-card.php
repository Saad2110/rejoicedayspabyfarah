...
class WC_Product_PW_Gift_Card extends WC_Product_Variable {
    ...
    protected $extra_data = array(
        ...
        'pwgc_physical_email' => '',
        'pwgc_expire_days' => '',
        'pwgc_product_ids' => array(),
        'pwgc_exclude_product_ids' => array(),
        'pwgc_product_categories' => array(),
        'pwgc_exclude_product_categories' => array(),
        'pwgc_exclude_tax' => false,
        'pwgc_exclude_shipping' => false,
    ...
    public function get_pwgc_product_ids( $context = 'view' ) {
        return $this->get_prop( 'pwgc_product_ids', $context );
    }

    public function get_pwgc_exclude_product_ids( $context = 'view' ) {
        return $this->get_prop( 'pwgc_exclude_product_ids', $context );
    }

    public function get_pwgc_product_categories( $context = 'view' ) {
        return $this->get_prop( 'pwgc_product_categories', $context );
    }

    public function get_pwgc_exclude_product_categories( $context = 'view' ) {
        return $this->get_prop( 'pwgc_exclude_product_categories', $context );
    }

    public function get_pwgc_exclude_tax( $context = 'view' ) {
        return $this->get_prop( 'pwgc_exclude_tax', $context );
    }

    public function get_pwgc_exclude_shipping( $context = 'view' ) {
        return $this->get_prop( 'pwgc_exclude_shipping', $context );
    }
    ...
    public function set_pwgc_product_ids( $pwgc_product_ids ) {
        $this->set_prop( 'pwgc_product_ids', $pwgc_product_ids );
    }

    public function set_pwgc_exclude_product_ids( $pwgc_exclude_product_ids ) {
        $this->set_prop( 'pwgc_exclude_product_ids', $pwgc_exclude_product_ids );
    }

    public function set_pwgc_product_categories( $pwgc_product_categories ) {
        $this->set_prop( 'pwgc_product_categories', $pwgc_product_categories );
    }

    public function set_pwgc_exclude_product_categories( $pwgc_exclude_product_categories ) {
        $this->set_prop( 'pwgc_exclude_product_categories', $pwgc_exclude_product_categories );
    }

    public function set_pwgc_exclude_tax( $pwgc_exclude_tax ) {
        $this->set_prop( 'pwgc_exclude_tax', $pwgc_exclude_tax );
    }

    public function set_pwgc_exclude_shipping( $pwgc_exclude_shipping ) {
        $this->set_prop( 'pwgc_exclude_shipping', $pwgc_exclude_shipping );
    }
    ...
    public function get_bonus_amount( $items, $item ) {
        ...
        if ( $this->get_pwgc_cumulative_bonus() ) {
            ...
            if ( pwgc_is_first_gift_card( $items, $item ) ) {
                foreach ( $items as $loop_item ) {
                    ...
                    if ( isset( $loop_item['product_id'] ) && $loop_item['product_id'] == $item['product_id'] ) {
                        $card_amount = is_a( $loop_item, 'WC_Order_Item' ) ? wc_get_order_item_meta( $loop_item->get_id(), PWGC_AMOUNT_META_KEY ) : 0;
                        if ( !empty( $card_amount ) ) {
                            $total += ( $card_amount * $loop_item['quantity'] );
                        } else {
            ...
            $card_amount = is_a( $item, 'WC_Order_Item' ) ? wc_get_order_item_meta( $item->get_id(), PWGC_AMOUNT_META_KEY ) : 0;
            if ( !empty( $card_amount ) ) {
                $total = $card_amount;
            } else {
    ...
    public function sync_gift_card_amount_attributes() {
        ...
        foreach( $variations as $variation ) {
            ...
            $variation_attributes = $variation->get_attributes();
            if ( $amount == 0 ) {
                $variation_attributes[ PWGC_DENOMINATION_ATTRIBUTE_SLUG ] = pwgc_get_other_amount_prompt( $this );
            } else {
                $variation_attributes[ PWGC_DENOMINATION_ATTRIBUTE_SLUG ] = $pw_gift_cards->pretty_price( $amount );
            }
    ...
    protected function sync_other_amount_variation() {
        ...
        foreach ( $variations as $index => $variation ) {
            if ( is_a( $variation, 'WC_Product' ) ) {
                ...
                if ( !empty( $variation_attributes ) ) {
                    ...
                    // This prevents creating multiple Other Amount variations when there are no attributes found.
                    $custom_amount_variation = true;
                }
            }
        }
        ...
        } else if ( is_a( $custom_amount_variation, 'WC_Product' ) ) {
